if (input && dv) {
    obsidian = input.obsidian;
    if (input.filterKeys) {
        filterKeys = input.filterKeys;
    } else {
        filterKeys = [];
    }

    var prop = require(app.vault.adapter.basePath + '/assets/javascript/dataview/npe_renderer.js');
    let view = dv.container;
    const currentFile = dv.current().file;
    const fileCache = app.metadataCache.getFileCache(currentFile);
    if (fileCache.frontmatter['ELN version'] === '0.4.2') {
      // add heading to the view
      view.createEl('h2', { text: 'Device Properties' });
    }
    prop.renderFrontMatter(obsidian, view, fileCache.frontmatter, 'device', filterKeys);
}